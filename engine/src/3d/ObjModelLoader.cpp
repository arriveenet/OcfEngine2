...
bool ObjModelLoader::load(std::string_view fileName, Mesh& mesh)
{
    ...
    for (size_t s = 0; s < shapes.size(); s++) {
        ...
        arrays[Mesh::ArrayType::Vertex]    = std::move(positions);
        arrays[Mesh::ArrayType::Normal]    = std::move(normals);
        arrays[Mesh::ArrayType::TexCoord0] = std::move(texCoords);
        arrays[Mesh::ArrayType::Index]     = std::move(indices);
